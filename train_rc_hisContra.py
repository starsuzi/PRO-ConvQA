...
from scripts.preprocess.convert_squad_to_hf_hisContra import convert_squad_to_hf
...
def main():
    ...
    # TODO
    previous_question_column_name = "previous_question" if "previous_question" in column_names else column_names[-1]
    ...
    def prepare_train_features(examples):
        ...
        # TODO
        examples[previous_question_column_name] = [pq.lstrip() for pq in examples[previous_question_column_name]]
        ...
        # TODO
        tokenized_previous_questions = tokenizer(
            examples[previous_question_column_name],
            truncation="only_first",
            max_length=max_query_length,
            return_overflowing_tokens=False,
            return_offsets_mapping=False,
        ...
        # TODO
        # Inflate questions based on sample_mapping
        tokenized_examples['input_ids_previous_question'] = [tokenized_previous_questions['input_ids'][i] for i in sample_mapping]
        tokenized_examples['token_type_ids_previous_question'] = [tokenized_previous_questions['token_type_ids'][i] for i in sample_mapping]
        tokenized_examples['attention_mask_previous_question'] = [tokenized_previous_questions['attention_mask'][i] for i in sample_mapping]
    ...
    if args.do_train:
        ...
        if args.truecase:
            for idx in range(len(train_dataset)):
                ...
                # TODO
                pq = train_dataset[idx][previous_question_column_name]
                train_dataset[idx][previous_question_column_name] = truecase.get_true_case(pq) if pq == pq.lower() else pq
    ...
    if args.do_eval:
        ...
        if args.truecase:
            for idx in range(len(eval_examples)):
                ...
                # TODO
                pq = eval_examples[idx][previous_question_column_name]
                eval_examples[idx][previous_question_column_name] = truecase.get_true_case(pq) if pq == pq.lower() else pq
    ...
    if args.do_predict:
        ...
        if args.truecase:
            for idx in range(len(predict_examples)):
                ...
                # TODO
                pq = predict_examples[idx][previous_question_column_name]
                predict_examples[idx][previous_question_column_name] = truecase.get_true_case(pq) if pq == pq.lower() else pq
    ...
    #import pdb; pdb.set_trace()
    #tokenizer.decode(train_dataset['input_ids_previous_question'][1])
    #tokenizer.decode(train_dataset['input_ids_'][1])
