...
def main(args):
    ...
    for qid, my_key in tqdm(enumerate(my_pred.keys())):
        pred = my_pred[my_key]
        #import pdb; pdb.set_trace()
        my_dict = {"id": my_key, "question": None, "answers": [], "ctxs": []}
        ...
        #import pdb; pdb.set_trace()
        assert len((pred["evidence"])) == len(pred["evidence"]) == len(pred["title"]), "Should use opt2 for aggregation"
